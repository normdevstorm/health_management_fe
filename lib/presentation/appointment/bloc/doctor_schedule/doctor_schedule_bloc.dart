...
class DoctorScheduleBloc
  ...
  DoctorScheduleBloc(this._doctorScheduleRepository)
      : super(DoctorScheduleState.initial()) {
    on<GetDoctorScheduleEvent>(
        (event, emit) => _onGetDoctorScheduleEvent(event, emit));
  }

  _onGetDoctorScheduleEvent(
      GetDoctorScheduleEvent event, Emitter<DoctorScheduleState> emit) async {
    emit(DoctorScheduleState.loading());
    try {
      final doctorSchedule = await _doctorScheduleRepository.getDoctorSchedule(
