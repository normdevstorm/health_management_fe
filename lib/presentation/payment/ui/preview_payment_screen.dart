...
class PreviewPaymentScreen extends StatefulWidget {
  ...
  const PreviewPaymentScreen({required this.appointment, super.key});
...
class _PreviewPaymentScreenState extends State<PreviewPaymentScreen> {
  ...
  Future<void> _proceedToPayment(BuildContext context) async {
    ...
    try {
      ...
      getIt<Logger>().i('ZaloPay API Response: ${response.toJson()}');
      ...
      final result = await ZalopayService.payOrder(zpTransToken);
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildInfoRow(
                            'Doctor'.tr(),
                            widget.appointment.doctor?.firstName != null
                                ? '${widget.appointment.doctor!.firstName} ${widget.appointment.doctor!.lastName ?? ""}'
                                : 'Unknown Doctor'),
                        SizedBox(height: 8.h),
                        _buildInfoRow(
                            'Health Provider'.tr(),
                            widget.appointment.healthProvider?.name ??
                                'Unknown Provider'),
                        SizedBox(height: 8.h),
                        _buildInfoRow(
                            'Note for doctor'.tr(),
